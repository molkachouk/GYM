...
export default function CustomerHome() {
    ...
    axios
        .get('http://localhost:8080/test/customer/viewGyms')
        .then(response => {
            localStorage.setItem('gyms', JSON.stringify(response.data));
        });
    axios
        .get('http://localhost:8080/test/customer/viewPrograms')
        .then(response => {
            localStorage.setItem('allPrograms', JSON.stringify(response.data));
        })
    axios
        .get('http://localhost:8080/test/customer/viewTrainers')
        .then(response => {
            localStorage.setItem('trainers', JSON.stringify(response.data));
        })
    axios
        .get('http://localhost:8080/test/customer/viewAppointments')
        .then(response => {
